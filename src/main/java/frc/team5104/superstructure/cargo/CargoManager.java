...
public class CargoManager extends BreakerSubsystem.Manager {
	...
	public static BezierCurveInterpolator beltInterpolator = new BezierCurveInterpolator(0.05,
			new BezierCurve(0.4, 0.2, 0.0, 1));

	public void update() {
		if (Climber.isClimbing())
			currentState = CargoState.idle;

		Chute.BeamAverage.update(ChuteSystems.BeamBreak.isHit());

		if (Arm.isManual() && currentState != CargoState.eject) {
			if (Chute.BeamAverage.getBooleanOutput() && ArmSystems.LimitSwitch.isHit())
				Cargo.idle();
			else if (Chute.BeamAverage.getBooleanOutput())
				Cargo.idle();
			else if (!ArmSystems.LimitSwitch.isHit())
				Cargo.intake();
			else Cargo.idle();
		}
