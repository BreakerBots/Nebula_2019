/*BreakerBots Robotics Team 2019*/
package frc.team5104.subsystem.vision;

public class _VisionConstants {
	public static double _maxXOffset = 27.0; // Outer X limit
	public static double _maxYOffset = 20.5; // Outer Y limit
	public static double _minXOffset = 0.5;  // Where to cap the adjustment
	public static double _p = 1;			 // Tune as needed for good results
	public static double _defSpeed = 0; //0.5;	 // Default speed, tune as needed
	
	// Pipeline indices
	public static int _line = 0;
	public static int _target = 1;
}
